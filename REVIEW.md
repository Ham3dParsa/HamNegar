...
## بدنه PR و تست دستی — تفکیک انسان و عامل

- **عامل (AI) موظف به تست صوتی نیست.** تست 5 ثانیه فارسی (`سلام، این آزمایش هم‌نگار است`) و پیست لاگ وظیفه انسان است (`AGENTS.md:3`). بازبینی نباید از عامل انتظار `log-panel` واقعی در PR body داشته باشد.
- **بازبینی باید کیفیت بدنه را چک کند، نه وجود لاگ صوتی.** بدنه PR باید با مهارت `unslop` نوشته شود: فارسی طبیعی و صمیمی، بدون puffery و بدون لیست بولدِ تکراری، با ریتم متنوع و نظر شخصی. باید شامل: چی درست شد، تغییرات فنی با `file:line`، و بخش `## تست دستی — با انسان` با دستورالعمل برای انسان باشد.
- **معیار قبولی PR body:** آیا بدنه با `unslop` خواناست و دستور تست برای انسان دارد؟ اگر بله، تله `missing manual test evidence` نباید `REQUEST_CHANGES` بدهد؛ فقط اگر بدنه خالی یا بی‌کیفیت بود `REQUEST_CHANGES`.
