...
## 3. قرارداد
- تغییر دامنه قبل از کد: سؤال ساختاری (کی به کی وصل است) اول با `graphify query` روی گراف محلی (اسکیل `graphify-index`)؛ جزئیات دقیق بعد با `grep -rn` روی `js/modules` و `read` همان گره‌ها.
