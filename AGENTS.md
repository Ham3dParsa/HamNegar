...
## 7. انتشار (release)
- با هر تغییر رفتاریِ مرج‌شده: `VERSION` در `js/modules/version.js` بالا برود و ورودی `CHANGELOG.md` در همان PR بنشیند (نه بعداً).
- متن‌های راهنما و کمک (`hint`ها، لیبل‌ها، راهنمای شورت‌کات) در همان PR بازبینی شوند؛ PR رفتاری بدون بازبینی متن ممنوع.
- شواهد تصویری: هر تیکت UI شات قبل/بعد زیر `docs/evidence/<issue>/` داخل همان PR دارد؛ ایشو با لینک raw و بستن بعد از مرج.
