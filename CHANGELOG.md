# Changelog — هم‌نگار

همه تغییرات مهم این پروژه اینجا ثبت می‌شود. فرمت بر اساس [Keep a Changelog](https://keepachangelog.com/fa/1.0.0/) و نسخه‌گذاری `MAJOR.MINOR.PATCH`.

## [0.6.44] - 2026-09-11
### Fixed
- **گارد زمان‌نمای فانتوم، ستون اپ (#144):** خروجی پالایش پیش از نمایش/اعمال در شیت بازبینی از تایم‌کدهای تنهای STT (۰:۲۴، 12:34، ۰۱:۱۵) پاک می‌شود؛ اعداد بدون کولن (ورژن ۱۳، ۱۰۰ کارت) و ساعت واقعی پس از «ساعت» دست‌نخورده؛ پرامپت‌های پالایش هم «بدون تایم‌کد» شدند. فقط `js/modules/stagebar.js` (نیمه STT با ستون همکار). شواهد: `docs/evidence/144/*`.

## [0.6.43] - 2026-09-11
### Changed
- **رفتار تاشو کنار نوار اقدام (#110 T1d، اصلاح جای T1c):** `#behavior-strip` بالای `#stagebar` حذف شد (جای بد) و همان ۴ چیپ (شناسه‌ها و متن‌ها و `checked`ها بایت‌به‌بایت) به پاپ‌اور `#behavior-pop` کنار فوتر `.actionbar` آمد؛ دکمه تک‌تیک `#behavior-toggle` (‏۳۶px، `‹`/`›` با `aria-expanded`) باز/بسته می‌کند، پیش‌فرض همیشه بسته (بدون persist، سیم فقط `index.html`+`css/app.css`+`app.js`)؛ persist هر ۴ تاگل و گیت پالیش/آنی و صفر-بودن رفتار در مودال دست‌نخورده. دسکتاپ: `gap`/`padding` جزیره فقط به‌اندازه جا شدن دکمه ۸ام تنگ شد (اسکرول افقی صفر). شواهد: `docs/evidence/110/t1d-*`.

## [0.6.42] - 2026-09-11
### Added
- **قرص شناور، حالت ضبط (#147):** صفحه جدای `pill.html` (دایره ۶۴px قابل‌درگ، تایمر فارسی، موج زنده، بج جفتی موتور، نقطه سهمیه، حباب نتیجه با کپی) با سیم‌کشی تازه `js/modules/pill.js` روی همان زنجیره STT — دکمه کنسل حین ضبط و ارسال + Esc، کلیدهای M/H، بدون دزدیدن فوکوس؛ صفر تغییر در فایل‌های موجود. شواهد: `docs/evidence/147/*`.

## [0.6.41] - 2026-09-11
### Changed
- **رفتار به صفحه اصلی (#110 T1c، اصلاح T1+T1b):** سکشن «🎚️ رفتار» به‌طور کامل از مودال تنظیمات حذف شد (نه بالا نه ته — هیچ `behavior-title` و هیچ تاگلی در مودال نیست) و همان ۴ چیپ (آنی/VAD/کپی خودکار/پالیش نهایی — شناسه‌ها و متن‌ها بایت‌به‌بایت) به نوار باریک `#behavior-strip` در صفحه اصلی آمد (تک‌سطر فشرده بالای `#stagebar`، نزدیک شست و میک، ارتفاع ۳۸px)؛ persist هر ۴ تاگل و گیت پالیش/آنی دست‌نخورده (همان `getElementById`، بدون تغییر JS). شواهد: `docs/evidence/110/t1c-*`.

## [0.6.40] - 2026-09-11
### Fixed
- **رفتار به ته مودال (#110 T1b، اصلاح جای T1):** سکشن «🎚️ رفتار» (۴ چیپ: آنی/VAD/کپی خودکار/پالیش نهایی) از بالای پنل‌ها (بین تب‌ها و `#panel-pipeline` — خوانده‌شدن به‌عنوان سربرگ پایپ‌لاین) به ته مودال تنظیمات آمد (بعد از `#panel-wave`، بالای hint/دکمه‌ها)؛ در هر دو تب فقط با اسکرول به ته دیده می‌شود؛ شناسه‌ها و متن‌ها بایت‌به‌بایت، persist هر ۴ تاگل دست‌نخورده؛ بدون تغییر JS/CSS. شواهد: `docs/evidence/110/t1b-*`.

## [0.6.39] - 2026-09-11
### Fixed
- **نوار اصلی استاتیک مگر زنده (#91 T3، اصلاح #102):** نوار اصلی `#rec-strip` در بیکاری و پس از توقف ضبط خط تخت استاتیک است (دیگر با fake نمی‌رقصد)؛ حرکت فقط با analyser واقعی زنده (میکروفون/مسیر صوتی زنده)؛ `setFakeEnabled(false)` فقط روی نمونه اصلی در `app.js` و گیت `!analyser && !fakeOn → خط تخت` در `wave.js` (مسیر fake-ON پیش‌نمایش/استارترهای تب تنظیمات بایت‌به‌بایت دست‌نخورده). شواهد: `docs/evidence/91/t3-*`.

## [0.6.38] - 2026-09-11
### Fixed
- **نوار پیش‌نمایش زنده + جداسازی از گیت کلید (#105):** مارکاپ `#live-preview`/`#live-final`/`#live-interim` (حذف‌شده در 0de6c96) به `index.html` برگشت — درست بعد از نوار وضعیت — با استایل قبلی در `css/app.css` (نمایش فقط با کلاس `on`، interim کم‌رنگ با زیرخط چین‌دار)؛ ارجاع‌های زنده `app.js` دیگر guard خاموش نیستند. ضبط بی‌کلید وقتی `realtime=1` و WebSpeech پشتیبانی شود مجاز است با بنر «پیش‌نمایش زنده بدون کلید؛ ثبت نهایی نیاز به کلید»؛ توقف به‌جای traceback 401، پرامپت کلید (باز شدن ⚙️) نشان می‌دهد و متن interim زنده در خروجی می‌ماند (کلیدِ وسط ضبط، مسیر عادی batch را ادامه می‌دهد). بدون تغییر schema — فقط `index.html` + `css/app.css` + سیم‌کشی `app.js`. شواهد: `docs/evidence/105/*`.

## [0.6.37] - 2026-09-11
### Fixed
- **سطح خطای واقعی مسیر realtime (‎#106):** مرورگر بدون پشتیبانی حالا hint ماندگار + toast + خاموش‌شدن خودکار تاگل می‌دهد (پیش‌تر «🔴 در حال ضبط... (زنده)» دروغین بود)؛ مقدار برگشتی `start()` بررسی و شکست شروع سطح‌بندی می‌شود؛ کدهای `onError` به status/toast فارسی متمایز راه‌یافته‌اند (`not-allowed`/`service-not-allowed`/`audio-capture` کشنده بدون تلاش مجدد، `network` با تلاش مجدد، `no-speech`/`aborted` فقط لاگ)؛ `onend` حین ضبط تا ۵ بار با backoff خطی (۸۰۰ms‎×تلاش) ری‌استارت می‌شود و بعد hint ماندگار می‌دهد — بدون ضبط ابدی سکوت؛ ضبط (مسیر Audio) در همه حالت‌ها ادامه دارد. فقط سیم‌کشی `app.js` و مسیر خطای `realtime.js` (افزایشی، بدون بازسازی موتور). شواهد: `docs/evidence/106/*`.

## [0.6.36] - 2026-09-11
### Fixed
- **نوار دوخطی موبایل (#112 T2):** در عرض ≤420px نوار مراحل (`stage-row`) دوخطی با ترتیب اولویت آیکون‌ها و بدون اسکرول افقی (tail صفر در ۳۶۰/۳۹۰) و نوار اقدام در بدترین حالت (تایمر+لغو) بدون سرریز (tail صفر در ۳۶۰)؛ `scope` همچنان پنهان (قانون 390px دست‌نخورده)؛ فقط CSS، بدون تغییر JS/رفتار؛ دسکتاپ ۱۴۴۰ بی‌تغییر. شواهد: `docs/evidence/112/t2-*`.

## [0.6.35] - 2026-09-11
### Changed
- **خروج رفتار از پایپ‌لاین (#110 T1):** سکشن «🎚️ رفتار» (۴ چیپ: آنی/VAD/کپی خودکار/پالیش نهایی) از داخل `#panel-pipeline` به سطح مودال تنظیمات آمد (بالای هر دو پنل، بیرون تب‌ها) — در تب موج هم دیده می‌شود؛ شناسه‌ها و متن‌ها بایت‌به‌بایت، گیت پالیش (`toggle-polish` → `polishEnabled`) و persist هر ۴ تاگل دست‌نخورده؛ بدون تغییر JS/CSS. شواهد: `docs/evidence/110/t1-behavior/*`.

## [0.6.34] - 2026-09-11
### Fixed
- **سقف کیبوردی textarea (‎#112 T1):** بازشدن کیبورد (resize با ویوپورت کوچک‌شده) حالا فقط کوچک می‌کند، هرگز رشد نمی‌دهد؛ سقف `min(40vh ویوپورت کوچک‌شده، 280px در عرض ≤420px)` و latch دائمی دستیِ PR #98 هم تسلیم سقف کیبوردی می‌شود (درگ همچنان دستی می‌کند، دابل‌تپ همچنان ریست)؛ select/focus هیچ autogrowای نمی‌زنند (delta 0px)؛ دسکتاپ ۱۴۴۰ بی‌تغییر. شواهد: `docs/evidence/112/t1-*`.

## [0.6.33] - 2026-09-10
### Changed
- **نمایش جفتی ارائه‌دهنده/مدل (#28):** همه رشته‌های نمایشی مسیرهای STT/پالیش/ترجمه به شکل `providerId/modelId` (مثل `groq/qwen3.6-27b` و `google/gemini-flash-lite-latest`) با helper متمرکز `pairLabel`؛ منطق انتخاب و fallback و کلید دست‌نخورده؛ `listModels` و داک پیشرفت سیم جداست.

## [0.6.32] - 2026-09-10
### Fixed
- **افوردنس کارت لایه (#109 T4):** دکمه شورون صریح (`▾`، `aria-expanded`) برای باز/بسته + دکمه ✎ جدا برای تغییرنام؛ تپ روی عنوان فقط باز/بسته می‌کند (دیگر ورودی تغییرنام باز نمی‌کند)؛ Esc ورودی تغییرنام را بدون ثبت می‌بندد؛ شورون در حالت بسته ۹۰° چرخیده (`◂`، راست‌به‌چپ)؛ دکمه‌های جابه‌جایی/حذف (قاب‌دار) از ناحیه شورون/تغییرنام (بی‌قاب) متمایزند. شواهد: `docs/evidence/109/t4-*`.

## [0.6.31] - 2026-09-10
### Added
- **کلید قطع سراسری موج (#109 T3):** تیک ماندگار «پنهان‌سازی انیمیشن صدا» (`wave-hide` در سراسری پشته) نوار `#rec-strip` را به ارتفاع صفر می‌برد و هر سه حلقه موج (نوار اصلی، پیش‌نمایش تب، استارترها) را می‌خواباند؛ معکوس پرچم `waveIdle` از PR #113 (خواندن/نوشتن فقط، بدون تغییر `storage.js`)؛ با همان تیک برمی‌گردد، حاشیه‌های recording/transcribing جدا می‌مانند، `prefers-reduced-motion` مستقل و دست‌نخورده. شواهد: `docs/evidence/109/t3-*`.

## [0.6.30] - 2026-09-10
### Fixed
- **گارد ضدلوپ بات بازبینی (#127، #128):** `opencode-review` دیگر روی push خود بات (`opencode-agent[bot]` و هر `*[bot]`) اجرا نمی‌شود؛ run دوم `synchronize` که با «does not have write permissions» فیل می‌شد و دو کامنت تکراری می‌گذاشت حذف شد. `opencode` (تریگر `/oc`) هم actor بات را صریح رد می‌کند چون لینک `opencode.ai` داخل کامنت بازبینی خودش زیررشته `/opencode` دارد.

## [0.6.29] - 2026-09-10
### Fixed
- **رفع چسبندگی روی‌هم (#109 T6):** `top` پیش‌نمایش sticky موج (`.wave-sticky`) از `0` به `56px` رفت تا زیر تب‌های تنظیمات (`top:0`) بچسبد و روی آن‌ها رنگ نگیرد؛ تب‌ها کلیک‌پذیر ماندند. شواهد: `docs/evidence/109/t6-*`.

## [0.6.28] - 2026-09-07
### Changed
- **جابه‌جایی منبع پیش‌نمایش (#109 T5):** سکشن «منبع پیش‌نمایش» (`mic-test` + `fake-toggle` + `sens-mini`) از ته تب موج به زیر پیش‌نمایش sticky و بالای استارترها آمد؛ متن/رفتار/شناسه‌ها بایت‌به‌بایت، فقط ترتیب مارکاپ؛ فاصله از `gap` موجود `#panel-wave`، بدون تغییر CSS/JS. شواهد: `docs/evidence/109/t5-*`.

## [0.6.27] - 2026-09-07
### Fixed
- **بهداشت تنظیمات مرده موج (#109 T2):** لغزنده `ضخامت` روی نوع `bars` پنهان شد (در `drawBars` خوانده نمی‌شود)؛ `barShape/barCount/barGap` فقط روی `bars` و `رنگ ۲` فقط با `gradient` رندر می‌شوند (پیش‌تر هم شرطی بودند — حالا با هوک `data-hygiene` قابل‌آزمون)؛ لغزنده `ته‌رنگ زمینه` فقط وقتی `aurora` روشن است دیده می‌شود (`waveSyncHue` + قانون `.wslider[hidden]`)؛ `حساسیت این موج` صادقانه شد: «گین پیش‌نمایش — گیت را عوض نمی‌کند» (انتخاب label: گیت‌کردن ریسک رفتاری داشت، `shown()` فقط گین می‌دهد). شواهد: `docs/evidence/109/t2-*`.

## [0.6.26] - 2026-09-07
### Added
- **پوشش دیکشنری در حلقه داخلی (#27، ادامه #23):** خروجی موفق polish در حلقه داخلی `transcribe` و `polishText` هم از دیکشنری شخصی رد می‌شود (فقط لایه polish، دیکشنری خالی no-op)؛ مسیرهای fallback وقتی پالیش خاموش یا زنجیره خالی است دست‌نخورده.

## [0.6.25] - 2026-09-07
### Added
- **اتصال دیکشنری به خط پالیش (#23):** خروجی موفق لایه `polish` در `textChain` از `applyPersonalDictionary` با `Storage.getDict()` رد می‌شود (لایه `translate` دست‌نخورده)؛ دیکشنری خالی no-op بایت‌به‌بایت؛ حالا که `getDict` در main است، گارد غیبت هم پوشش واقعی دارد.

## [0.6.24] - 2026-09-07
### Removed
- **حذف ذن از سیم رونویسی (#25):** `queryResponsesText` و `testZenspark` و شاخه ذن در `listModels` و شناسه‌های `muse-spark-*` کامل پاک شدند؛ providerId کانونیک `google` در همه مسیرها (endpoint و هدر همان)؛ شناسه ناشناس (از جمله ذن باقی‌مانده در UI تا موج بعد) خطای تایپ‌دار 400 می‌دهد نه crash؛ ورودی‌های ذن ذخیره‌شده پیش‌پرواز فیلتر می‌شوند. زنجیره STT و بودجه و 413 و دیکشنری دست‌نخورده.

## [0.6.23] - 2026-09-07
### Changed
- **شناسه کانونیک ارائه‌دهنده (#24):** `gemini`→`google` (نام توسعه‌دهنده، مثل `openrouter`)؛ `zenspark` از سیم Storage purged شد (ورودی‌های زنجیره‌اش در نرمالایز دور ریخته می‌شوند، مقدار خام کلید یتیم می‌ماند)؛ اسلات‌های localStorage بایت‌به‌بایت پس کلید هیچ کاربری نمی‌پرد؛ زنجیره‌های legacy در خواندن migrate می‌شوند. پیش‌فرض پالیش از لیست زنده رایگان Groq: `openai/gpt-oss-120b` اول، بعد `qwen3.8-27b` و `qwen3.6-27b` و `gpt-oss-20b`.

## [0.6.22] - 2026-09-07
### Fixed
- **آیکون‌های متمایز stagebar (هاتفیکس روی #115، بخشی از #110):** هر ۶ دکمهٔ `.sbtn` به‌جای گلیف ایموجی (تکراری ✨ برای ساده/پیشرفته) آیکون SVG خطی متمایز گرفت — ساده=تک‌ستاره، پیشرفته=ستاره+لغزنده‌ها، دستوری=متن+تیک، ترجمه سریع=پیکان‌های دوسویه، پنل ترجمه=پنجره+سطرها، خام=بازگشت+نقطه (`stroke=currentColor` ‏۱۹px، الگوی `.cbtn`)؛ `title`/`aria-label` کامل فارسی از `GUIDE`، ارتفاع لمسی ۴۰px و سرریز ‎≤65px@360 دست‌نخورده؛ بدون تغییر JS.

## [0.6.21] - 2026-09-07
### Fixed
- **ورودی طولانی ابزارها (#111):** بودجه خروجی از طول ورودی مقیاس می‌گیرد (کف ۲۰۰۰، سقف ۸۱۹۲) به‌جای سقف ثابت؛ برش طول (`finish_reason length` / `MAX_TOKENS`) خطای متمایز «متن طولانی» (`413`) می‌دهد؛ strip حریص `<think>` نابسته فقط وقتی پاک می‌کند که متن قابل‌استفاده قبلش باشد (بدنه برش‌خورده هرگز به `''` نمی‌رسد). ورودی کوتاه و مسیر STT دست‌نخورده.

## [0.6.20] - 2026-09-07
### Changed
- **آیکونی‌شدن ابزارهای stagebar (#110 T2):** هر ۶ دکمهٔ `.sbtn` آیکون‌خالص شدند (گلیف موجود در `‎.s-ic` با `aria-hidden`؛ نام کامل فارسی از `GUIDE` در `title`/`aria-label` — هاور دسکتاپ نام را نشان می‌دهد) با الگوی `.cbtn` نوار اقدام (‏`svg+title+aria` روی هر ۸ دکمه)؛ `title=null` صفر از ۶؛ سطر stage در ۳۶۰ از ‎358px به ‎65px سرریز (۳۹۰: ‎328→‎35؛ ‏۱۴۴۰: صفر)؛ ارتفاع لمسی ‎40px و تاگل پنل ترجمه و رفتار `hidden` دست‌نخورده؛ بدون تغییر JS.

## [0.6.19] - 2026-09-07
### Fixed
- **سقف زنجیره (#112 T3/3):** `.chain-list` سقف گرفت — موبایل `240px` (حدود ۳ سطر + لبهٔ سطر ۴ به‌عنوان affordance اسکرول، قد واقعی سطر ۶۶–۹۹px)، دسکتاپ `≥1024px` دقیق ۶ سطر (`calc(6 * 66px + 5 * 6px)` = ‏۴۲۶px)؛ اسکرول داخلی `overflow-y:auto` mirror الگوی add-list (‏`css/app.css:682`) پس دنبالهٔ ۸ سطری بدون اسکرول مدال در دسترس است؛ سطرها، منوی ⋮ و سوییچ‌ها دست‌نخورده. شواهد: `docs/evidence/112/t3-*`.

## [0.6.18] - 2026-09-07
### Changed
- **سربرگ فشرده لاگ T2/2 (#90):** هدر موبایل `≤640px` دوسطری شد — سطر ۱ تک‌سطر (سربرگ ellipsis + اکشن‌های آیکونی ۴۴×۴۴ با `title`/`aria-label`؛ متن دسکتاپ سر جاست)، سطر ۲ نوار فیلترها (nowrap + اسکرول افقی، `min-height:28px`)؛ جستجو همچنان اورلی صفرپیکسل؛ `#log-body` در پنل پیش‌فرض 180px حدود ۳٫۱ سطر (کف: ۳ در ۳۹۰، ۲٫۵ در ۳۶۰)؛ دسکتاپ ۱۴۴۰ پیکسل‌به‌پیکسل (سطر ۴۳px). تکمیل T1: نگه‌دارنده containing-block اورلی + `stopPropagation` در Esc جستجو.

## [0.6.17] - 2026-09-07
### Changed
- **نوار اصلی صدا زنده شد (#91 T1):** `.rec-strip` قد ۴۸→۶۸px (‏`flex-shrink:0` و `overflow:hidden` سر جاست، هاله بریده نمی‌شود)؛ نوار اصلی بدون میک با fake می‌تپد (‏`setFakeEnabled(true)` تا analyser بنشیند — اتصال analyser اولویت دارد، رفتار full-level و خط ثابت `reduced-motion` دست‌نخورده). حاشیه‌های recording/transcribing دست‌نخورده.

## [0.6.16] - 2026-09-07
### Fixed
- **خوانایی قدم‌های STT در موبایل (#91 T2):** پنل `#stt-progress` کف `96px` با `flex-shrink:0` گرفت (سقف ۱۱۰px و `hidden` سر جاست) + محافظ شرینک نوار؛ نام‌های نمایشی انسانی (`Groq`، `Flash-Lite 3.5`…) با ارقام فارسی (شمارندهٔ `قدم i از n` همیشه پیدا، بدون ellipsis در ۳۶۰)؛ سربرگ ۱۳px؛ هدر لاگ در `≤640px` تک‌سطر شد (اسکرول افقی درون‌خطی) تا `#log-body` در ۳۹۰ حداقل ۵ سطر جا بدهد. شکل فراخوانی‌های `transcription.js` دست‌نخورده.

## [0.6.15] - 2026-09-07
### Changed
- **جستجوی لاگ آیکونی T1/2 (#90):** `#log-search` همیشه‌روشن حذف شد؛ دکمهٔ 🔍 در `#log-actions` (‏۴۴×۴۴، `aria-expanded`/`aria-controls`، حلقهٔ فوکوس) فیلد را به‌صورت اورلی باز می‌کند؛ Esc/✕ می‌بندد، کوئری و فوکوس حفظ می‌شود؛ هدر موبایل `≤640px` تک‌سطر (‏۵۷px در ۳۹۰) و دسکتاپ ۴۳px بدون تغییر؛ رفتار `Logger.log` و `hidden` دست‌نخورده.

## [0.6.14] - 2026-09-07
### Changed
- **شیت راهنما در موبایل (#94):** دیالوگ راهنما در `≤640px` شیت تمام‌عرض چسبیده به کف شد (`align-end`، `85dvh`، شعاع بالای ۲۰px، زبانه‌های چسبان + دکمهٔ تمام‌عرض بستن — هم‌ارز شیت بازبینی)؛ جدول میانبرها به سطرهای انباشتهٔ کارتی شکست؛ ترتیب Esc و یادداشت Enter آکاردئون (`details` بومی، بدون تغییر JS)؛ دسکتاپ تا ۶۴۰px عریض شد؛ `prefers-reduced-motion` رعایت شد؛ تله/aria/بستن با تپ پس‌زمینه دست‌نخورده.

## [0.6.13] - 2026-09-07
### Fixed
- **لمس تغییراندازه رونوشت (#93):** `.grip` حالا `touch-action:none` با هدف لمسی ≥۴۴px، مدیریت `pointercancel`/capture به‌علاوهٔ فallback لمسی `touchstart` (الگوی splitter)؛ پرچم manual-override (درگ می‌نشاند؛ autogrow فقط رشد می‌دهد، هرگز ارتفاع کاربر را کوچک نمی‌کند؛ دابل‌تپ بازنشانی می‌کند)؛ `@media(pointer:coarse){#output{resize:none}}` (دسکتاپ هندل native را نگه می‌دارد). `Storage.saveHeights({out})` دست‌نخورده. درگ لمسی واقعی ±۶۰px در هر سه ویوپورت بدون دزدیدن اسکرول، بدون خطای کنسول.

## [0.6.12] - 2026-09-07
### Added
- **محتوای واقعی راهنما (#95):** دیالوگ راهنما ۴ تب شد (میانبرها + ابزارها/تنظیمات/سهمیه) با تک‌منبع حقیقت = مپ `GUIDE` در `js/app.js` (هر درایه `{id,title,body,ref}` به شناسهٔ موجود؛ خروجی زبانهٔ میانبرها دست‌نخورده)؛ `title` کنترل‌های مقید از همان مپ می‌آید + قانون ضدپوسیدگی (هر PR روی stagebar/settings/quota باید مپ را لمس کند).

## [0.6.11] - 2026-09-07
### Fixed
- **لنگر splitter لاگ (#92):** حذف `position:sticky;bottom:0;z-index:5` از قانون `#log-splitter` (فقط `order:5` می‌ماند) — دستگیره حالا خواهرِ static-flow لبه پنل است، نه سنجاق‌شده به کف اسکرول‌پورت موبایل. شکاف `splitter.top − panel.bottom` در هر سه ویوپورت (۳۶۰/۳۹۰/۱۴۴۰) هم در scrollTop صفر و هم در انتها ۸px است؛ درگ لمسی ±۸۰px پنل و دستگیره را با هم حرکت می‌دهد؛ جمع‌شدن پنل دستگیره را پنهان می‌کند. ریاضی درگ/کلَمپ/ذخیره‌سازی دست‌نخورده.

## [0.6.10] - 2026-09-07
### Added
- **نوع ستونی اکولایزر + پیش‌نمایش زنده استارترها (#57):** نوع هفتم `bars` کنار ۶ نوع خطی (ستون‌های عمودی مرکز-آینه با همان سطوح باند، شکل گرد/مربع/سوزنی، تعداد ۸–۴۸، فاصله ۰–۸؛ ذخیره‌سازی با پیش‌فرض امن برای ورودی‌های قدیمی) + استارتر یازدهم «اکولایزر ستونی»؛ کارت‌های استارتر حالا بوم‌های زنده (~۱۲۰×۴۸) با یک rAF مشترک ~۱۶fps، ساعت مصنوعی مشترک، توقف با IntersectionObserver و `document.hidden`، و فریم ثابت `renderOnce()` زیر `prefers-reduced-motion`.

## [0.6.9] - 2026-09-07
### Changed
- **پیش‌نمایش و لغزنده‌های موج (#56):** پیش‌نمایش بلندتر (۲۰۰px دسکتاپ / ۱۵۰px موبایل) با جاگذاری عمودی (`ampOf` ۰٫۵۲→۰٫۴۰) ضد برش قله؛ نوار چسبان خلوت (فقط پیش‌نمایش + شمار) و کنترل‌های منبع زیر تاخوردگی؛ یک سازنده `waveSlider()` با شست قطره‌ای، حباب مقدار، تیک‌ها و دابل‌کلیک بازنشانی — جای هک `min=-1` چیپ «همگام با سراسری» + ترک ۰–۱۰۰؛ سراسری‌ها شیشه‌ای متمایز از رونوشت‌های هر موج.

## [0.6.8] - 2026-09-07
### Changed
- **تفاوت چهارحالته شیت بازبینی:** حذف‌شده قرمز خط‌خورده، افزوده خالص آبی، جایگزینی (DEL+INS چسبیده) زرد/کهربایی با زیرخط، دست‌نخورده سفید. توکنایزر نقطه‌گذاری را از واژه جدا می‌کند (نیم‌فاصله نمی‌شکند) تا نقطه‌گذاریِ عوض‌شده پس از واژه سالم، تنها هایلایت شود.

## [0.6.7] - 2026-09-06
### Added
- **راهنمای میانبرهای درون‌برنامه (#51):** دکمه `؟` در نوار اقدام + کلید `؟/?` بیرون از ورودی‌های متن، دیالوگ راهنما را باز می‌کند. تک‌منبع حقیقت = مپ `SHORTCUTS` در `js/app.js` (رندر در دیالوگ + `title`/`aria-keyshortcuts` روی کنترل‌های مقید)؛ محتوا دقیقاً بایندینگ‌های واقعی + ترتیب لایه‌ای Esc (راهنما → پنل ترجمه → شیت بازبینی → مدال → لغو ضبط) + یادداشت «Enter همیشه زمینه‌ای، هرگز سراسری».

## [0.6.6] - 2026-09-06
### Fixed
- **حوزه شورت‌کات‌ها (#50):** Ctrl/Cmd+Z/Y فقط با فوکوس `#output` (بقیه ورودی‌ها undo بومی + گارد چیدمان فارسی via `e.code`)؛ درج رونویسی صریح `editorHistory.push()` می‌کند (پایان data-loss)؛ Esc لایه‌ای با consumption (پنل ترجمه → شیت diff → مدال → لغو ضبط در آخر).

## [0.6.5] - 2026-09-06
### Added
- **شیت تأیید تفاوت (#49):** هر پالایش/ترجمه حالا اول شیت پایین (متن اصلی/نتیجه با هایلایت کلمه‌ای، شمار تغییرات، اعمال/دورریختن) را باز می‌کند؛ اعمال همان مسیر قبلی (`stagePushRaw`+`stageApply`، سهمیه فقط روی اعمال)، دورریختن هیچ پشته‌ای را لمس نمی‌کند. Enter=اعمال، Esc=دورریختن.

## [0.6.4] - 2026-09-06
### Changed
- **افزودن درون‌زنجیره‌ای (#76):** هر زنجیره دکمه بازشونده (`aria-expanded`) + پنل جست‌وجو زیر همان زنجیره دارد؛ افزودن بدون اسکرول، با اعلام و بدون بستن پنل. شیت انتخاب مدل و `activePickerTarget` حذف شدند؛ پنل STT مدل‌های متنی را پنهان می‌کند (پایان ردیف‌های میوت کاذب).

## [0.6.3] - 2026-09-06
### Fixed
- **ردیف‌های فشرده زنجیره (#75):** نام کامل، تک‌توکن وضعیت، منوی ⋮ (بدون تکثیر — زنجیره‌ها مجموعه‌اند).

## [0.6.2] - 2026-09-06
### Fixed
- **پیشرفت فشرده (#78):** فقط ۲ قرص جاری+بعدی، سرخط قدم و نام مدل، سقف ۱۱۰px.

## [0.6.1] - 2026-09-06
### Fixed
- **ترتیب عمق جزیره (#77):** `z-index` جزیره ۱۳۰ بالای نوار میانی، پارک اسکرول محتوا، سقف ارتفاع منوها. در حالت متن بلند ممکن است منو هنوز به جزیره برسد (با ترتیب رنگ درست) — پیگیری جدا.

## [0.6.0] - 2026-09-06
### Added
- **پایپ‌لاین هوشمند (#38، #39):** دو زبانه (پایپ‌لاین + موج صدا)، هاب ارائه‌دهندگان، زنجیره‌های STT/پالیش زیر هاب، کشوی بسته کلیدها و شیت انتخاب مدل با جست‌وجو.
- **شیت تأیید افزودن + هاب سفارشی (#58):** کلیک چیپ سفارشی کشو و فرم را باز و نام را فوکوس می‌کند.
- **گزارش استفاده تک‌زبانه (#66) + بازیابی آمار (#71):** دو/سه هیرو (درخواست، دقیقه/کلمه، سرعت)، اسپارک‌لاین بدون عدد، کارت هر مدل با خط فارسی، کپشن ذخیره.
- **کارت‌های مدل خلوت (#61):** فقط نام + توانایی + رایگان، دکمه سبز/قرمز عضویت، بلاک راهنما، idهای بلند wrap.
- **لیست‌ها با اسکرول داخلی + خطای نام‌دار (#62):** سقف ~۵/~۸ ردیف، `✕ <ارائه‌دهنده>:`، راهنمای دستی Zen.
- **meta viewport `viewport-fit=cover` (#42):** اینست‌های safe-area روی iOS واقعی شدند.
- **شواهد تصویری (#65 به بعد):** هر تیکت شات قبل/بعد زیر `docs/evidence/<issue>/` دارد.
### Changed
- **توکن‌های شیشه‌ای (#37):** متریال Liquid Glass، مدال شیشه‌ای، دکمه‌های کپسولی، فوکوس و reduced-motion.
- **جزیره شناور اکشن‌بار (#40) + گارد ناچ (#41):** شناور ۵۴۰px با blur، حاشیه safe-area، شیشه stagebar.
- **فوتر چسبان مدال + گرید زنجیره (#43):** ذخیره همیشه دیده، ردیف دومرحله‌ای تا ۴۸۰px، کنتراست سراسری.
- **حذف هاب (#63) و هدر (#65):** کشو-اول با auto-expand بی‌کلید؛ چرخ‌دنده در اکشن‌بار، رفتار+پالیش در مدال، موتور در scope.
- **شفافیت نوار ابزار (#67):** لیبل‌های صادقانه، پانوشت session-only، اسکرول ۳۹۰.
- **چیپ‌های رفتار فشرده (#70):** سوییچ استاندارد هم‌زبان chain-item.
- **اسلitter لاگ (#59):** پین زیر پنل با sticky.
### Fixed
- **میوت موج (#60):** resolve شیء زنده با id — رفت‌وبرگشت بی‌صدا/باصدا.
- **تاچ ۳۶px و wrap شناسه‌ها (#61، #43).**

## [0.5.0] - 2026-09-03
### Added
- **Wave personalization (ticket/50):** new 4th settings tab «موج صدا» — sticky live preview (mic test + fake + sensitivity), 10 starter cards (one-click stack fill), wave stack ≤5 with per-wave rows (title toggle + rename + quick controls + collapsed advanced: band/profile/overrides/c2), globals (sensitivity/attack/speed/intensity/smooth/particles/aurora), 🎲 random stack + reset. Persisted as stack JSON v3 under `hamnegar.wave.v3` (`Storage.getWave/saveWave`).
- **Wave engine (`js/modules/wave.js`):** standing-wave renderer (gate + floor-freeze hysteresis, perc loudness curve, near-still idle, no horizontal travel, thickness coupling, band drivers low/mid/high/rms, profiles flat/center/edges/bands, layered glow without per-frame `shadowBlur`, DPR-aware ≤60fps rAF, reduced-motion static). Main-page rec strip now runs on this engine with the user's saved stack.
- **Main redesign (ticket/51):** slim secondary top bar (brand-mini + version badge + engine chip + options drawer + gear), rec strip driven by `wave.js` + user stack (live via `Audio.getAnalyser()`, near-still idle), merged one-line status (dot + text + chars + words), autogrow transcript (cap ~60vh + grip resize), bottom sticky action bar with 88px round mic, collapsed quota strip (today numbers + worst-provider dot, auto-expand on 429 only), collapsed log (auto-expand on error with toast).

## [Unreleased] — Post-merge gate compliance (follow-up for PR #4)

### Added
- **Gate compensation:** follow-up branch `fix/followup-pr4-gate` addressing premature merge of PR #4 (`eb666f4`) with 5 must-fix warnings open and direct-to-`main` fix `82fe4aa` that bypassed review gate (violates `AGENTS.md`/`REVIEW.md`). This PR re-documents the bypass in `README.md` (§ Post-merge gate compliance) and here, and records evidence for the 5 fixes in `REVIEW.md` + `docs/PR4_FOLLOWUP_EVIDENCE.md`.
- **Policy:** future merges require OC review `APPROVED`; no direct `main` pushes for review fixes.

### Fixed (applied in 82fe4aa, now gate-tracked here)
- `js/app.js:123` XSS esc, `js/modules/quota.js:29` sttChain seam, `js/modules/transcription.js:114-123` 401 skip, `js/modules/storage.js:40` parseChain filter, manual 5s transcription test obligation.

## [0.3.1] - 2026-09-02
### Fixed
- باگ متن طولانی در حالت آنی: `fin` تجمعی باعث تکرار از ابتدا می‌شد — حل با `snap.committed = fin` (cumulative) به جای `+=`
- حذف کادر اضافی لایو (باکس نقطه‌چین) که متن را دو بار نشان می‌داد
- مدل `gemini-2.5-flash-lite` منسوخ (404) → جایگزینی با `gemini-3.5-flash-lite` و `gemini-3.1-flash-lite` و `gemini-flash-lite-latest`
- اسپم لاگ `DEBUG final`: فقط وقتی `fin` واقعا جدید شد لاگ می‌زند

## [0.3.0] - 2026-09-02
### Added
- معماری ماژولار (`storage`, `transcription`, `audio`, `realtime`, `quota`, `logger`) بر اساس `codebase-design`
- حالت آنی با `Web Speech API` + `VAD` و `quota` زنده
- حافظه پیش‌نویس و ارتفاع پنل‌ها در `localStorage`
- پنل لاگ با فیلتر سطح و اسپلتر دستی
- پشتیبانی کلید `AQ.` با هدر `x-goog-api-key`

### Changed
- نام پروژه از `voice-assistant-modular` به `هم‌نگار` (HamNegar) — سازنده Ham3dParsa

## [0.2.0] - 2026-09-02
### Fixed
- `run.bat` — `pushd` و باز کردن مستقیم `index.html`، رفع `Directory listing`
- `file://` بنر هشدار CORS

## [0.1.0] - 2026-09-02
### Added
- نسخه اولیه تک‌فایل `gemini-code-*.html` با Groq Whisper + Gemini
